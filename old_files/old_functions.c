...
void gather_haloes()
{
	...
	if(ThisTask==0)
//		fprintf(stdout, "\nGathering %d haloes into one structure...\n", NHaloes);

		{		
			haloes = (struct halo *) calloc(NHaloes, sizeof(struct halo));
			SizeDispl[0] = 0;
		}	

		MPI_Gather(&pSettings[ThisTask].n_haloes, 1, MPI_INT, SizeItems, 1, MPI_INT, 0, MPI_COMM_WORLD);	

		if(ThisTask==0)
			for(n=1; n<NTask; n++)
				SizeDispl[n] = SizeItems[n-1]+SizeDispl[n-1];
		
		//	MPI_Scatter(&SizeItems, NTask, MPI_INT, &SizeItems, NTask, MPI_INT, 0, MPI_COMM_WORLD);
		//	MPI_Scatter(&SizeDispl, NTask, MPI_INT, &SizeItems, NTask, MPI_INT, 0, MPI_COMM_WORLD);
		//	MPI_Barrier(MPI_COMM_WORLD);

		if(ThisTask==0)
			for(n=0; n<NTask; n++)
			{	
				fprintf(stdout, "Task=%d, SizeItems[%d]=%d\n", ThisTask, n, SizeItems[n]);
				fprintf(stdout, "Task=%d, SizeDispl[%d]=%d\n", ThisTask, n, SizeDispl[n]);
			}

//		create_mpi_halo_datatype();
//		MPI_Gatherv(&(pHaloes[ThisTask][0]), SizeItems[ThisTask]*sizeof(struct halo), MPI_BYTE, 
//				haloes, SizeItems, SizeDispl, MPI_BYTE, 0, MPI_COMM_WORLD);
//		MPI_Gatherv(&(pHaloes[ThisTask][0]), SizeItems[ThisTask], MPI_HALO, 
//				haloes, SizeItems, SizeDispl, MPI_HALO, 0, MPI_COMM_WORLD);
